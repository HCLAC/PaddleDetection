...
<view class="audio-player" @tap="audioPlay" :style="'opacity: ' + opcity">
	...
	<view class="controls play-btn" @tap="audioPlay">
		...
		<image :src="style == 'self'? '../../../static/images/yuyin.png' : '../../../static/images/yuyin.png'"></image>
	</view>
</view>
</template>
...
export default {
  ...
  destroyed() {
    let audioCtx = this.$data.__comps__.audioCtx = audioCtxFc.getCtx(this.msg.id);
  ...
  methods: {
	...
	if(uni.getSystemInfo().app === 'alipay'){
      ...
      uni.downloadFile({
        ...
        header: {
          ...
          Accept: "audio/mp3",
          Authorization: "Bearer " + this.msg.token
        },
    ...
    audioPause(auCtx) {
      //let audioCtx = this.data.__comps__.audioCtx;
      let audioCtx = this.$data.__comps__.audioCtx = audioCtxFc.getCtx(this.msg.id) || auCtx;
